...
- **Costo figurativo lavoro soci** (`progetto.lavoro_soci = { attivo, ragguaglio, righe:[{id,nome,ore,tariffa}] }`): costo-opportunità del lavoro dei soci non retribuiti = Σ ore×tariffa. Tenuto **fuori dal CE civilistico e dalle imposte**; alimenta solo il *reddito normalizzato* (Utile netto − costo figurativo), i KPI di produttività oraria (ricavi/ora, MdC/ora) e un break-even che remunera anche i soci. Blocco separato in fondo alla vista Budget.
  - **Ore annue a regime + ragguaglio primo anno** (`lavoro_soci.ragguaglio`, default true): le `ore` inserite sono annue a regime (12 mesi); per società con `mese_avvio > 1` il primo esercizio è parziale e le ore vengono ragguagliate ai mesi operativi (fattore `mesi_operativi/12`) nel calcolo del costo figurativo. Con avvio a gennaio il fattore è 1 (nessun effetto). Toggle "Ragguaglio ON/OFF" nel blocco soci.
  - **Coerenza di orizzonte del reddito normalizzato** (`calcolaBudget`, engine.js): il reddito normalizzato deve sottrarre il costo soci da un utile sullo **stesso orizzonte temporale**, altrimenti si mescolano mesi parziali e anno pieno. Due ottiche gestite dal toggle:
    - **Ragguaglio ON** → *ottica primo esercizio parziale*: costo soci ragguagliato ai `mesi_operativi`, sottratto dall'utile del budget parziale (avvio→dic). Nessuna annualizzazione.
    - **Ragguaglio OFF su anno parziale** (`mese_avvio > 1`) → *ottica a regime*: costo soci **pieno annuo**, e l'utile del budget parziale viene **proiettato a 12 mesi** con fattore `12/mesi_operativi` (proiezione lineare, coerente col pro-rata lineare del budget). Anche le basi del break-even soci (`kRim`, `fissiBE`) e il fatturato/MdC dei KPI orari sono annualizzati con lo stesso fattore, così tutto resta comparabile. Con avvio a gennaio il fattore è 1 (ON e OFF coincidono → retrocompatibile).
    - Campi engine: `reddito_norm_utile_base` (utile usato: parziale o proiettato), `reddito_norm_a_regime` (bool, per etichettare la UI), `reddito_norm_fattore`. La UI Budget etichetta la riga "Utile netto proiettato a 12 mesi" in ottica a regime; il Consuntivo annualizza allo stesso modo `pre.proiezione.utileNetto` e `pre.fatturato_proiettato` nelle card soci (reddito normalizz. proiettato + Δ break-even).
  - **Soci nel Consuntivo**: quando il conteggio è attivo, l'head del Consuntivo espone tre riquadri KPI aggiuntivi — *Costo figurativo soci*, *Reddito normalizzato proiettato* (utile netto proiettato − costo soci) e *Break-even coi soci* (con Δ vs fatturato proiettato).
  - **KPI MdC/ora a semaforo** (`_mdcOraInfo`, ab/ui.js): il riquadro "MdC / ora vs tariffa" confronta il margine di contribuzione orario con la **tariffa media dei soci** (`lavoro_soci.tariffa_media = costo/ore_effettive`, invariante d'orizzonte). Tre livelli con colore e sub-etichetta dinamici: `lvl-ok` (≥1,3× → verde, "+X/h oltre il compenso"), `lvl-warn` (1,0–1,3× → arancio, "copre appena"), `lvl-bad` (<1,0× → rosso, "sotto la tariffa").
  - **Simulazione compenso amministratori** (`lavoro_soci.simula_compenso` bool + `lavoro_soci.fisco = { prelievo_socio_pct, ricarico_azienda_pct }`; engine `calcolaCompensoNettoSoci`): what-if **separato dal CE e dalle imposte del budget**. Trasforma ore×tariffa (compenso lordo annuo a regime, su 12 mesi, ore non ragguagliate) in **netto in tasca al socio** e **costo pieno per l'azienda**, con due % medie indicative editabili (stima rapida, non scaglioni puntuali): `netto = lordo × (1 − prelievo_socio_pct)` (default 0,35 = IRPEF+addizionali+INPS quota socio), `costo_azienda = lordo × (1 + ricarico_azienda_pct)` (default 0,20 = INPS 2/3 c/società + IRAP indeducibile). Toggle "Simula compenso amministratori" nel blocco soci del Budget → pannello con aliquote + tabella per-socio (Lordo/Netto/Costo azienda mensili) + totali. Effetto fiscale reale (se il compenso venisse davvero pagato): IRES ↓ (compenso e contributi deducibili), IRAP invariata (compensi amministratori indeducibili IRAP). Handler: `Projects.lavoroSociSimula`, `Projects.lavoroSociFisco`.
    - **Base del reddito normalizzato legata alla simulazione** (`calcolaBudget`, campi `costo_azienda_soci`, `netto_soci`, `costo_reddito_soci`, `simula_compenso`): quando la simulazione è **OFF** (default) il reddito normalizzato e il break-even soci sottraggono il costo figurativo puro (`costo_figurativo_soci = ore×tariffa`, costo-opportunità — retrocompatibile). Quando è **ON**, la base diventa il **costo pieno azienda** (`costo_reddito_soci = costoFigSoci × (1 + ricarico_azienda_pct)`, ottica "libro paga"): il riepilogo Budget mostra "− Costo soci a libro paga (azienda)" e una riga informativa "di cui netto in tasca ai soci" (`netto_soci = costoFigSoci × (1 − prelievo_socio_pct)`). Il prelievo del socio **non** entra nel costo azienda (è già dentro il lordo). Il Consuntivo rispecchia: card "Costo soci a libro paga" (valore = costo azienda, netto socio nel tooltip) e reddito/BEP sulla stessa base. L'annualizzazione a regime (`fattAnnua`) resta applicata all'utile, mentre il costo (figurativo o azienda) è già ragguagliato/pieno secondo il toggle Ragguaglio.
- **Layout riquadri**: nel Budget la card KPI di destra è larga 240px e il footer (seed + soci) occupa la sola colonna 1 (non `1 / -1`) per non finire coperto dalla card sticky quando si scrolla. Nel Consuntivo i toggle Frequenza/Distribuzione/Rimanenze sono resi come riquadri compatti (`.ab-cons-ctrl`) allineati ai riquadri KPI (`.ab-cons-kpi`) in un'unica barra `.ab-consuntivo-bar`.
- **Mese di avvio — unica sede nel Budget**: il selettore "Mese di avvio attività" vive nel pannello "Basi del budget" (vista Budget), **sempre visibile** (anche per società con storico). Nel Consuntivo resta solo il riflesso (mesi pre-avvio non editabili, denominatore "periodi operativi").
- **Creazione neocostituita (0 anni di storico)**: la modale "Nuova analisi" consente `anni_storici = 0` (stepper min 0) e ha il campo **"Mese di avvio attività"** (`na-mese-avvio` → `meta.mese_avvio`). Per una neocostituita (`anni_storici` vuoto) Budget e Consuntivo si renderizzano **anche senza import del CE** (guardie rilassate via `_abSenzaStorico`): il budget si costruisce dai soli valori ipotizzati/override. **L'import resta comunque disponibile** (es. stampa infrannuale di confronto): `applicaImportCE` popola lo storico dagli anni presenti nel file. L'entrata è sempre l'Import CE; lo Storico, finché non c'è import, mostra un messaggio che rimanda a Import o Budget.
- **Orizzonte budget del primo anno parziale**: quando `mese_avvio > 1` il budget dell'anno in corso rappresenta il **primo esercizio parziale** (avvio→dic, N mesi); il preconsuntivo confronta sullo stesso orizzonte operativo. Il pannello "Basi del budget" lo dichiara esplicitamente (etichetta `.ab-seed-horizon`). Il valore ×12 "a regime" del seed è solo un riferimento di pianificazione, non la base di confronto dell'anno in corso.
- **Import infrannuale per neocostituite** (`Projects.applicaImportInfrannuale`): per un progetto senza storico (`anni_storici` vuoto) l'import del CE è trattato come *infrannuale* — l'anno più recente del file è l'**anno in corso** (dati parziali), non uno storico chiuso. Nell'anteprima di import compare il campo **"Dati aggiornati fino a"** (mese di riferimento). Da lì: `mesi_coperti = mese_riferimento − mese_avvio + 1`, `fattore = mesi_operativi / mesi_coperti`. L'import scrive: budget → `override_pct` = incidenze % (variabili, scale-free), `override_fissi` = valori × fattore (fissi/imposte proiettati avvio→dic), `fatturato_ipotizzato` = ricavi × fattore; consuntivo → fatturato YTD ripartito in parti uguali sui mesi avvio→riferimento (poi override manuale). `anni_storici` resta vuoto; le rimanenze (stock) sono riportate as-is. `meta.import_infrannuale` traccia anno/mese/mesi_coperti. Il flusso di import normale (progetti con storico) resta invariato (`applicaImportCE`).
  - **Visibilità dei valori parziali importati**: nella **Mappatura sottoconti** le colonne-valore usano gli anni presenti nei `sottoconti_ce` quando `anni_storici` è vuoto (`_anniConValori`), così i dati importati restano visibili/editabili. Nel **Budget** la colonna "Ultimo anno €" si trasforma in **"avv–rif €"** (sigle mesi avvio–riferimento) e la "% storica" in **"% avv–rif"**: mostrano i **valori parziali realmente importati** (ricalcolati da sottoconti+mapping via `ExcelImport.calcolaStorico` sugli anni dei sottoconti — non `ricalcolaStorico`, che userebbe `anni_storici` vuoto), affiancati alla proiezione a fine anno nella colonna "Budget €". I totali di riga usano `_totaliDaValori`. Così l'operatore confronta *fatto finora* vs *previsto a fine anno* e può rettificare i sottoconti (Mappatura) o gli override (Budget).
...
- **PDF mensile orizzontale** (`esportaPdfConsuntivoMensile` → `_renderConsuntivoMensilePdfHtml` + `_buildPdfPeriodColsCompleto`, pulsante "📄 PDF mensile (orizz.)"): riproduce lo **sviluppo per periodo come a video** — 12 colonne mese (Gen→Dic) in mensile, 4 in trimestrale — affiancate alle colonne di sintesi Budget / Proiezione fine anno / Δ. I mesi non compilati sono in grigio (`.ab-pdf-col-vuoto`), i mesi precedenti all'avvio attività barrati (`.ab-pdf-col-preavvio`). Stampa in **landscape**: `_printPdf(html, title, { landscape: true })` inietta `@page { size: landscape }` (solo keyword, senza dimensione), così l'operatore sceglie **A4 o A3** nella finestra di stampa del browser — la tabella sta in A4 e respira in A3. Layout in `.ab-pdf-tab-mensile` con larghezze **in percentuale** + `table-layout: fixed` + `width: 100%`: la tabella riempie l'intera pagina, quindi in A3 le colonne (e lo spazio per le cifre) si allargano automaticamente. **Cifre del corpo** (mesi + sintesi) più grandi delle testate — `tbody td.num` a 9,5px in A4 — e **ulteriormente ingrandite in A3** (11,5px) via `@media print and (min-width: 340mm)`, che Chromium valuta sulla dimensione del foglio scelto (A4 orizz. ~297mm, A3 ~420mm; miglioria progressiva, se ignorata restano le cifre già ingrandite valide in entrambi i formati). Le testate non sono toccate. Δ compatto (solo € con segno), la % resta nel PDF verticale.
