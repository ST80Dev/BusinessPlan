...
- **Creazione neocostituita (0 anni di storico)**: la modale "Nuova analisi" consente `anni_storici = 0` (stepper min 0) e ha il campo **"Mese di avvio attività"** (`na-mese-avvio` → `meta.mese_avvio`). Per una neocostituita (`anni_storici` vuoto) Budget e Consuntivo si renderizzano **senza import del CE** (guardie rilassate via `_abSenzaStorico`): il budget si costruisce dai soli valori ipotizzati/override. `onProgettoAperto` fa atterrare queste società direttamente su **ab-budget**; lo Storico mostra un messaggio dedicato.
