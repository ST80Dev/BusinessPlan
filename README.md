...
2. **Mappatura sottoconti** — riassegnare i sottoconti alle macroaree corrette (drag & drop, multi-selezione). Da qui è anche possibile **aggiungere una voce di conto a mano** ("+ Aggiungi voce": codice, descrizione, macroarea): utile per rettifiche post-import o per costi/voci nuovi dell'anno da budgetare che prima non c'erano (es. "Compenso soci"). La voce nasce a **0** sullo storico; il valore per l'anno di budget si imposta poi con l'**override del gruppo** (eventualmente creando un **Nuovo gruppo** dedicato). I mastri 61/80 (variazione rimanenze) non sono inseribili a mano perché confluiscono nel blocco rimanenze calcolato.
   - Il **codice è facoltativo**: se non lo si conosce ancora, lasciarlo vuoto → viene creato un segnaposto `NUOVO-N` modificabile in seguito.
   - Le voci **create a mano** hanno **codice e descrizione modificabili** direttamente dalla riga (le voci importate dall'Excel restano immutabili). Un conto o un gruppo si possono **eliminare** con la "×"; i **gruppi custom** si possono anche **rinominare** (icona ✎). I mastri e i gruppi **predefiniti** restano bloccati (non eliminabili/rinominabili)
